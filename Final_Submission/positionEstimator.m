%% Position estimator code made by Little Clopaths Team - Imperial College London
...
X1 = zeros(T*angle,98); 
X2 = zeros(T*angle,98); 
    if N==320 % first section of movements
        section=1;
        len = 0;
        for times=1:1:T
            for a=1:angle
        ...
        X = [X0,X1,X2]';
        N=size(X,2);
        % Get model parameters from positionEstimatorTraining
        ...
        err_knn(1,n)=temper;
% %     mdl=fitcnb(modelParameters.xdata, modelParameters.ydata);
        pred_angle=pred_knn;
...
%         lin_reg_bin = length(N)/98-(320/dt-1);
lin_reg_total = N;
        decodedPosX(:)= modelParameters.LINREG(pred_angle,1:lin_reg_bin).x_coeff;
        decodedPosY(:)= modelParameters.LINREG(pred_angle,1:lin_reg_bin).y_coeff;
